= Getting started with Bigtop and Wireguard
:navtitle: Getting started with Wireguard
:keywords: sql, engine, BigTop, docker, Wireguard
:description: Testing MixQL with docker compose and BigTop via VPM Wireguard
...
WireGuard® is an extremely simple yet fast and modern VPN that utilizes state-of-the-art cryptography. It aims to be faster, simpler, leaner, and more useful than IPsec, while avoiding the massive headache. We use https://github.com/linuxserver/docker-wireguard
...
You can get access to the all web services inside docker network through WireGuard VPN. One of the services of docker-compose is wg: VPN server (https://github.com/linuxserver/docker-wireguard).

The container will run in server mode and the necessary server and peer/client confs will be generated. The peer/client config qr codes will be output in the docker log. They will also be saved in text and png format under /config/peerX.

Use VPN-client (https://www.wireguard.com/install/) to connect to endpoint.

Use configuration from ./dockerfiles/wg/config/peerX.conf. Change Endpoint to real external IP of your server.

To rebuild configuration just delete content of ./dockerfiles/wg/config and restart docker-compose.
